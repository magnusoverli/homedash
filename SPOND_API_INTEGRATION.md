...
export class EnhancedSpondAPI {
  ...
  async getGroupStatistics(groupId: string): Promise<GroupStatistics> {
    return this.makeAuthenticatedRequest(`${this.baseUrl}groups/${groupId}/statistics`);
  }
  ...
  async createEventFromTemplate(
    ...
    return this.makeAuthenticatedRequest(`${this.baseUrl}sponds/from-template`, {
      method: 'POST',
  ...
  async initializeChat(capabilities?: string[]): Promise<ChatInitResponse> {
    const response = await this.makeAuthenticatedRequest(`${this.baseUrl}chat`, {
      method: 'POST',
...
export class SecureSpondCredentials {
  static async store(userId: string, credentials: SpondCredentials): Promise<void> {
  ...
  private static async encrypt(credentials: SpondCredentials): Promise<EncryptedCredentials> {
